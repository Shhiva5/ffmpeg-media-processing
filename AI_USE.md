...
1. I would like to use the FFmpeg libraries to detect whether a video stream is CFR 
   (constant frame rate) or VFR (variable frame rate), and then implement timestamp-aware
   frame requests. Could you suggest a suitable project around this idea?
...
3. "What all I need to know for completion of Part-A" — produced the
   ...
   implementation plan.
4. "Part-A code template" — the actual build: CMake project, vendoring
   nlohmann/json, `media_inspector.{h,cpp}`, `json_report.{h,cpp}`,
   `main.cpp`, `scripts/generate_fixtures.sh`, `tests/test_assertions.cpp`,
   then iteratively: installed FFmpeg dev headers, generated fixtures, ran
...
7. Templates for Part B and Part C, then iterate on the exercise by modifying
   those templates.
...
- Both fixtures were actually generated with `ffmpeg` and independently
  cross-checked with `ffprobe` (used only as a comparison oracle) 
  for frame count, frame rate, and picture-type distribution.
- The frame-request selection rule was validated against a case designed
  to catch the classic bug this kind of tool needs to avoid
